...
export default function ProductImage({images=[{url:''}]}) {
  ...
  return (
    <div className="flex flex-col justify-center">
        ...
        {images.map((image,index)=><img key={index} className={`${image.url===headImage.url && 'border-4'} h-[5vh] w-[4rem] sm:h-[15vh] sm:w-[8rem] md:h-[14vh] object-cover p-[0.2rem] rounded-md hover:border-2 cursor-pointer border-green`} onClick={()=>setHeadImage(images[index||0])} src={image.url} alt={image.filename}/>)}
        </div>
    </div>
  )
}
