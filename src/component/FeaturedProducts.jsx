        ...
        {products.slice(0,4).map(product =>{
            ...
            return(
                ...
                <div className="relative">
                  <img src={image} className={`h-40 w-[40rem] object-cover rounded-t-md`} onMouseOver={()=>handleHoverEnter(id)} onMouseLeave={()=>handleHoverOut(id)} alt={`featured products${name}`}/>
                  <Link to={`/products/${id}`} className={`${hoverState[id]?'visible':'invisible'} absolute top-1 right-1 bg-white p-1 text-darkRed rounded-full text-xl`}>
                    <BiExpand/>
                  </Link>
                </div>
                <h1 className='capitalize font-bold my-2'>{name}</h1>
                <p className='text-sm text-darkRed font-bold leading-8 tracking-wide'>${price}</p>
              </div>
            )
        })}
        </div>
